...
`race.html`は1920×1080固定レイアウトです。各ゲーム映像は**800×450px（16:9）**で、その直下に映像と重ならない**800×54px**のPlayer情報バーが付きます。P1 / P2が上段、P3 / P4が下段で、Slot全体は800×504pxです。Slotの座標は固定されており、空Slotを非表示にしても他Slotは移動しません。

| Slot | ゲーム映像（x, y, w×h） | Player bar（x, y, w×h） | Timer Crop / frame（x, y, w×h） |
| ---- | ----------------------- | ----------------------- | ------------------------------- |
| P1   | 144, 8, 800×450         | 144, 458, 800×54        | 746, 464, 190×42                |
| P2   | 976, 8, 800×450         | 976, 458, 800×54        | 1578, 464, 190×42               |
| P3   | 144, 568, 800×450       | 144, 1018, 800×54       | 746, 1024, 190×42               |
| P4   | 976, 568, 800×450       | 976, 1018, 800×54       | 1578, 1024, 190×42              |

左右の映像間隔は32px、左右外側余白は144px、上下外側余白は8pxです。各Player bar内ではP badgeがlocal座標(8, 6)・44×42px、Player identityが(64, 0)・300×54px、PB / Rankが(376, 0)・210×54px、Timer frameが(602, 6)・190×42pxです。Timer frameの内側は透明で、OBSのTimer Cropを隠しません。中央情報帯は**x=144, y=512, 1632×56px**で、Commentaryを左、CategoryとWRを隣接させて右側に表示します。Event名・ロゴとゲーム名は表示しません。
...
配信前に1920×1080で色付き映像矩形とTimer Cropを重ね、映像がPlayer barに隠れないこと、P badgeが映像上にないこと、Timer Cropが表示されること、中央情報帯が映像に重ならないことを確認してください。また、長い名前、Commentator最大3人、WRなし、未割当Slotも確認します。
